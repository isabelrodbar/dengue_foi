...
real phi[T]; // reporting rates for primary and secondary infections
...
real lambda_year_g[Ag,T];
real cum_lambda_g[Ag-1,T];
real susc_g[Ag-1, T];
real mono_g[Ag-1, T];
real mono_dat_g[A, T];
real poi_rate_g[A, T];
...
for (i in 1:T) {
    for (j in 1:Ag) {
       // print(ind_lambda[j,i])
    	lambda_year_g[j,i] = lambda[ind_lambda[j,i]] ;
	}
 }

//print(ind_lambda);
for (i in 1:T) {
    cum_lambda_g[1,i]=lambda_year_g[2,i];
    for (j in 2:(Ag-1)) {
    	cum_lambda_g[j,i]=cum_lambda_g[j-1, i] + lambda_year_g[j+1,i];
					 }
			}

for( i in 1:T) {
     for (j in 1:Ag-1) {
     	 susc_g[j,i]=exp(-4*cum_lambda_g[j,i]);
	 mono_g[j,i]=4*susc_g[j,i]*(exp(cum_lambda_g[j,i])-1);
	 }
}

for (i in 1:T) {
    for (j in 1:A) {
    	mono_dat_g[j,i]=mono_g[ind_age[j], i];
	}
}

for (i in 1:T) {
    for (j in 1:A) {
    	poi_rate_g[j,i]=mono_dat_g[j,i]*3*lambda[ind_lambda[1,T-i+1]]*phi[T-i+1]*C[j,i];
	}
}
